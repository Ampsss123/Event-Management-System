                                ...
                                <table class="table align-items-center table-flush table-hover" id="dataTableHover">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-center">Log Description</th>
                                            <th class="text-center">Log DateTime</th>
                                            <th class="text-center" style="width: 15%;">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php
                                        $sql="SELECT a.*, ul.* FROM tbladmin a
                                              LEFT JOIN user_log ul ON a.ID = ul.AdminID
                                              WHERE a.Status='1' ";
                                        $query = $dbh->prepare($sql);
                                        $query->execute();
                                        ...
                                        if($query->rowCount() > 0) {
                                            foreach($results as $row) {    
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        <span><?php echo htmlentities($row->LogDescription);?></span>
                                                    </td>
                                                    <td class="text-center">
                                                        <span><?php echo htmlentities($row->LogDateTime);?></span>
                                                    </td>
                                                    <td class="text-center">
                                                        <a href="#" class="edit_data btn btn-purple rounded" id="<?php echo ($row->ID); ?>" title="click for edit">
                                                            <i class="mdi mdi-pencil-box-outline" aria-hidden="true"></i>
                                                        </a>
                                                        <a href="userregister.php?delid=<?php echo ($row->ID);?>" onclick="return confirm('Do you really want to Delete ?');" title="Block this User" class="btn btn-danger rounded">
